...
export default function TouristTripDetailScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (trekId) {
          try {
            trek = await fetchTrekPublic(trekId);
          } catch {
            /* optional */
          }
        }
        const title = trek?.title ?? trek?.name;
        const image =
          trek?.heroImageUrl ?? trek?.coverImageUrl ?? trek?.thumbnailUrl ?? trek?.imageUrl;
        ...
        const mapped = mapTouristBookingRecord(row, title, image, {
          locationLabel: typeof loc === "string" ? loc : undefined,
          heroCaption: cap || undefined,
        });
  ...
  const openMessage = () => {
    const base = ENV.webBaseUrl;
    if (base) void WebBrowser.openBrowserAsync(`${base}/messages`);
    else if (booking?.guideName) void Linking.openURL(`mailto:?subject=Trek%20Guider%20—%20${encodeURIComponent(booking.tripTitle)}`);
  };
  ...
  const guideInitials = initialsFromName(booking.guideName ?? "G");
      ...
      >
        ...
        {booking.guideName ? (
          <View style={styles.card}>
            <Text style={styles.cardEyebrow}>YOUR GUIDE</Text>
            <View style={styles.guideRow}>
              <View style={styles.guideAvatar}>
                <Text style={styles.guideAvatarText}>{guideInitials}</Text>
              </View>
              <View style={styles.guideMeta}>
                <Text style={styles.guideName}>{booking.guideName}</Text>
                {booking.guideCompanyName?.trim() ? <Text style={styles.guideCo}>{booking.guideCompanyName}</Text> : null}
              </View>
              <Pressable onPress={openMessage} style={styles.msgBtn}>
...
const styles = StyleSheet.create({
  ...
  guideAvatarText: { fontSize: 16, fontWeight: "800", color: "#9a3412" },
